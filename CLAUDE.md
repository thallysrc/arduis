...
- **Isolation via `COMPOSE_PROJECT_NAME`** — a unique project name per worktree gives separate
  container names, networks, and volumes (an isolated DB per worktree for free). Pass it as an env var.
...
- **Base from `main`** — read the compose file from the `main` checkout, generate the override into the worktree dir.
- **Port offset** — auto-assign with a per-worktree offset (config `port_offset = 1000`) but **probe
  for free ports** before committing; persist the chosen ports per worktree so badges/URLs stay stable.
- **Teardown** — `docker compose -p <name> down -v` on worktree removal/hibernate; wire into both
  "conclude worktree" and app-exit handlers (first-class RAM requirement).
