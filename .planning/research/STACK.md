...
**Domain:** Lightweight Linux/GNOME desktop app orchestrating parallel AI coding agents (Claude Code) in git worktrees, with embedded VTE terminals; Flatpak-first distribution
**Researched:** 2026-06-08
**Confidence:** HIGH (core stack verified against GitLab tags, Arch packages, Flathub, PyGObject release notes)
...
**Bottom line:** the stack is well chosen and current. Only one concrete correction (fast_float version) and several reliability caveats around `flatpak-spawn --host` that must shape the architecture.
...
flatpak-builder --user --install --force-clean build-dir io.github.thallys.Arduis.yml
flatpak run io.github.thallys.Arduis

# --- Test deps (host venv, only for CI/unit tests outside the sandbox) ---
pip install pytest pytest-xvfb PyGObject-stubs ruff mypy
# system: xvfb (or mutter), dbus, gobject-introspection, gir1.2-vte-3.91 if testing outside flatpak
```

> Note: inside the Flatpak you do NOT pip-install PyGObject/GTK/VTE — they come from the SDK + the compiled VTE module. Host-side pip is only for tooling and any unit tests you run outside the sandbox.

## VTE Bundling in Flatpak (the load-bearing detail)

**Fact confirmed: VTE for GTK4 (Vte-3.91) is NOT shipped in `org.gnome.Platform`/`Sdk` 50, and is not in Ubuntu 24.04's apt as the GTK4 variant.** It must be compiled inside the manifest via flatpak-builder. The draft manifest does this correctly with three ordered modules:

1. **fast_float** (header-only, cmake) — required by VTE 0.84
2. **simdutf** (C++ lib, cmake) — required by VTE 0.84
3. **vte** (meson, `-Dgtk4=true -Dgtk3=false -Dvapi=false -Ddocs=false`)

Build order in the draft is correct (deps before VTE before app). Confidence: HIGH.

**Version corrections / hardening:**
...
This is the single biggest technical risk in the stack. The whole product runs the host's `zsh`/`claude`/`git`/`docker` from inside a sandbox via `flatpak-spawn --host`.
...
**Reliability caveats to design around (MEDIUM confidence — operational, from docs + ecosystem):**
...
**Recommendation:** treat `flatpak-spawn --host` as a thin RPC to the host and centralize it in one module (a `host.run()` / `host.spawn_pty()` wrapper). This is where the portability and reliability risk concentrates; isolate it so the AUR/.deb native builds (no sandbox) can swap it for direct execution.
...
**State detection (Degrau 4) — strongest finding:**
...
All confirmed against Docker docs; the user's plan is idiomatic. (Confidence: HIGH for the mechanism, MEDIUM for exact port-collision strategy.)
...
**Architecture implication:** the `host.run()` abstraction must branch on "am I sandboxed?" — Flatpak path uses `flatpak-spawn --host`, native AUR/.deb path executes directly. Detect via presence of `/.flatpak-info` or `FLATPAK_ID`.
...
---
*Stack research for: GNOME desktop AI-agent orchestrator with embedded VTE terminals*
*Researched: 2026-06-08*
