...
- [x] **Phase 9: Packaging (AUR + .deb)** - Team-installable on Ubuntu + Arch under real Wayland as native packages using the system VTE; Flatpak deferred to v2 (completed 2026-06-15 — hardware UAT PO-accepted as open risk per 09-HUMAN-UAT.md)
...
Plans:
- [x] 09-01-PLAN.md — Wave 0: install build/lint toolchain, fix metainfo (homepage + 1.0.0 release), generate SVG icon, write 3 packaging test files (native no-op / version / install-tree)
- [x] 09-02-PLAN.md — meson foundation (D-01/D-02): purelib install_sources of the arduis pkg incl. hooks data + generated bin/arduis launcher (no sys.path hack) + data XDG install
- [x] 09-03-PLAN.md — .deb (DIST-03): debian/ set wrapping meson via dh, Architecture: all, exact CLAUDE.md Depends; builds locally + lintian clean
- [x] 09-04-PLAN.md — AUR (DIST-02): PKGBUILD arch=any wrapping meson, exact Arch deps, no .install; README packaging section (D-06)
- [x] 09-05-PLAN.md — acceptance: full automated gate + live hardware UAT (clean install + Wayland launch on real Ubuntu + Arch, DIST-04/SC-3) (checkpoint)
